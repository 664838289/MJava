...
import mjava.op.util.ArithmeticType;
...
/**
 *    Generate ROR (Rational Operator Replacement) mutants --
...
public class ROR extends MethodLevelMutator {
    ...
    public ROR(CompilationUnit comp_unit) {
        super(comp_unit);
    }

    public void visit(BinaryExpr be, Object obj){
            if(ArithmeticType.isCompareOperator(be.getOperator())){
                injectError(be);
            }
        //in order generate effective injected error, ignore this case
        //else if(ArithmeticType.isCompareOperator(be.getOperator())){
        //   objectMutantGen(be);
        //}
        super.visit(be,obj);
    }
    ...
    private void injectError(BinaryExpr binEx){
        //in order generate effective injected error, we generate specific mutants
